...
- 一般会综合所有子节点
- 树形DP的实现类似后序遍历。
- 树形DP和线性DP有关联，有时候可以相互转化。

树形DP一般只有两个顺序：从上往下（不常见）、从下往上，因此，**表达式中不能同时存在子节点信息和父节点信息**（即无后效性）
...
尝试以 $O(n)$ 枚举每一个点，然后再通过dfs统计一遍答案，总计 $O(n^2)$
...
定义：
- `sz[t]`表示在1为根节点的树中，以`t`为根节点的子树的节点数量。
- `low[t]`表示在1为根节点的树中，以`t`为根的子树中所有节点到节点`t`的距离之和。
- `dp[t]`表示在`t`为根节点的树中，书中所有节点到节点`t`的距离。

`sz[t]`很好求，参考**树的大小**一节。

第一个dfs求`low`时，**从下到上**，有：
...
第二个dfs求`dp`时，考虑若将根节点从`u`改成`v`，`v`是`u`的子节点，则有以下变化：
- 不位于节点`v`下面的节点（不包括`v`，共计 $n-sz[v]$ 个节点）每一个到根节点的距离增大1
- 位于节点`v`下面的节点（包括`v`，共计 $sz[v]$ 个节点）每一个到根节点的距离减少1
...
和**01背包**相类似，定义`dp[t][k]`表示在以`t`为根的子树中，选择`k`门进行学习，所能够获得的最大分数。
...
利用`g`数组来优化`dp`的状态转移方程：$$dp[u][k] = g[v_{max}][k-1] + s[i]$$，$v_{max}$ 为节点 $u$ 的子节点数量。必须选取节点 $u$，因为如果不选取节点 $u$，节点 $u$ 中的所有子节点都无法被选取。
