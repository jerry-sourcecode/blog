...
由已知可得： $$\left\{ \begin{aligned} & k_1p+n=a \\ & k_2p+n=b\end{aligned} \right.$$其中，$$n,k_1,k_2\in\mathbb{Z}$$
所以：
...
对于整数 $a,b,c,d,p$，如果有： $$\left\{ \begin{aligned} & a\equiv b\pmod p \\ & c\equiv d\pmod p\end{aligned} \right.$$则有：$$(a+c) \equiv (b+d)\pmod p$$
证明：

由已知条件可得：
$$\left\{ \begin{aligned} & k_1p+n=a \\ & k_2p+n=b \\ & k_3p+m=c \\ & k_4p+m=d\end{aligned} \right.$$
其中，$$n,m,k_1,k_2,k_3,k_4\in\mathbb{Z}$$
所以： 
...
对于整数 $a,b,c,p$，如果有： $$a\equiv b+c\pmod p$$则有：$$a-c \equiv b\pmod p$$
证明：
...
对于整数 $a,b,p$，如果有： $$a\equiv b\pmod p$$且 $c$ 是 $a, b, p$ 的公因数，则有：$$\frac{a}{c} \equiv \frac{b}{c}\pmod {\frac{p}{c}}$$
证明：

由已知条件可得：
$$\left\{ \begin{aligned} & k_1p+n=a \\ & k_2p+n=b \end{aligned} \right.$$
其中，$$n,k_1,k_2\in\mathbb{Z}$$
所以：
...
同理可证，$$\frac{b}{c}\bmod \frac{p}{c}=\frac{n}{c}$$
所以：
...
对于整数 $a,b,c$，如果有： $$\begin{aligned} & ac\equiv bc\pmod p\end{aligned}$$则有：
$$a \equiv b\pmod p$$
...
\begin{aligned}
ac&\equiv bc\pmod p\\
ac\cdot c^{-1}&=bc\cdot c^{-1} \pmod p\\
a &\equiv b\pmod p
\end{aligned}
$$
...
对于正整数 $p$，全部整数可以分为 $p$ 个集合，令其为 $K_0,K_1,K_2,\cdots,K_{p-1}$，其中，整数 $n$ 和其所在的集合 $K_i$ 所满足的条件为：$$n\equiv i\pmod p$$
